...
//need a helper function to avoid copying of vector
template <class T, class C>
size_t _insertion(const std::vector<T>& elements, size_t start_index, size_t size, const C& val){
  ...
  if(size==1)
    return elements[start_index] < val ? start_index+1 : start_index;

  size_t half = size/2;

  if(elements[start_index+half]==val)  return (start_index+half);
  else if(elements[start_index+half]>val)
    return _insertion(elements, start_index, half, val);
  else
    return _insertion(elements, start_index+half, size-half, val);

} 
template <class T, class C>
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
  return _insertion(elements, 0, elements.size(), val);
}
