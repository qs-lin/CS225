...
HuffmanTree::HuffmanTree(vector<Frequency> frequencies)
{
    ...
    vector<bool> path;
    buildMap(root_, path);
    //std::cout << "map size: " << bitsMap_.size() << std::endl;
}
...
//remove one node from one queue
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while( (singleQueue.size() + mergeQueue.size() ) >= 2 ){
      ...
      TreeNode* merge = new TreeNode(left->freq.getFrequency() + right->freq.getFrequency());
      merge->left  = left;
      merge->right = right;
    ...
    TreeNode* node = removeSmallest(singleQueue, mergeQueue);
    root_ = node;
    //
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    TreeNode* node = root_;
    while (bfile.hasBits()) {
      bool result = bfile.getNextBit();
      
      //std::cout << static_cast<int>(result);
      //we hit a leaf
      if(node->left==nullptr && node->right==nullptr){
        ss << node->freq.getCharacter();
   //     std::cout << "char: " << node->freq.getCharacter() << std::endl;
        node = root_;
      }
      if(result==false)
        node = node->left;
    ...
    //we still have one last node
    //note that we always fetch the data from node when we go back to root first
    //but since bfile is empty now, so we cannot go back to root  
    //so right now we are stuck at the last leaf but we haven't fetch the data
    ss << node->freq.getCharacter();
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
    ...
    if(current->left==nullptr && current->right==nullptr){
      bfile.writeBit(static_cast<bool>(1));
      bfile.writeByte(current->freq.getCharacter());
    }else{  //find an internal ndoe
      //pre-order
/*
      bfile.writeBit(static_cast<bool>(0));
...
//void HuffmanTree::print(std::ostream& out = std::cout) const
{
    int h = height(root_);
