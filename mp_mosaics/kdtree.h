...
class KDTree
{
  ...
  private:
    void debug();
    void  buildTree(std::vector<Point<Dim>>&, size_t, size_t, KDTreeNode*&, size_t);
    double distance(const Point<Dim>& p1, const Point<Dim>& p2) const;
    //find the median in one dim and do the partition around this pivot
    size_t partition(std::vector<Point<Dim>>&, size_t, size_t, size_t ); 
    //Point<Dim> findMedian(std::vector<Point<Dim>>&, size_t, size_t, size_t, size_t ); 
    size_t findMedian(std::vector<Point<Dim>>&, size_t, size_t, size_t, size_t ); 
    //Point<Dim> findMedian(std::vector<Point<Dim>>&, size_t, size_t, size_t);
    size_t findMedian(std::vector<Point<Dim>>&, size_t, size_t, size_t);
    size_t ceil(size_t );  

    KDTreeNode* buildTree(std::vector<Point<Dim>>&);
    ...
    void clear();
    void _clear(KDTreeNode*);
    void copy(const KDTree<Dim>& other);
    KDTreeNode* _copy(KDTreeNode*);

    Point<Dim> findNNS(const Point<Dim>&, typename KDTree<Dim>::KDTreeNode*, size_t) const;
